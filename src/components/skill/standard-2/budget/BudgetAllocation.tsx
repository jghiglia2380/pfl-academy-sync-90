...
export function BudgetAllocation({ skillBuilder, onComplete }) {
  ...
  const [budgetData, setBudgetData] = useState(
    skillBuilder.sections[currentStep].initialBudgetData
  );
  ...
  const fetchData = async () => {
    ...
    if (response) {
      ...
      setBudgetData(JSON.parse(response.skillbuilder_json));
    }
  };
  ...
  const handleNext = async () => {
    if (currentStep < steps.length - 1) {
      ...
      const updatedData = await saveSkillBuilder(
        ...
        JSON.stringify(budgetData), // Convert selections to JSON
        user?.id, // User ID

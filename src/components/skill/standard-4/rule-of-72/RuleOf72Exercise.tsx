...
import RuleOf72Form from "./RuleOf72Form";
import {
  saveSkillBuilder,
  getSkillBuilder,
...
export function RuleOf72Exercise({ skillBuilder, onComplete }) {
  ...
  useEffect(() => {
    if (
      formData.aiResponse?.[1] &&
      formData.aiResponse?.[2] &&
      formData.aiResponse?.[3] &&
      formData.reflection?.[1] &&
      formData.reflection?.[2] &&
      formData.reflection?.[3]
    ) {
      setIsFormValid(true);
    } else {
  ...
  return (
    <div className="max-w-[1000px] mx-auto py-8">
      ...
      <div className="mb-8">
        {currentStep === 0 && (
          <RuleOf72Form formData={formData} setFormData={setFormData} />
        )}
      </div>

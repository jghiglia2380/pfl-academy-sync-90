...
import TaxSystemAnalysis from "./TaxAnalysis/TaxSystemAnalysis";
import {
  saveSkillBuilder,
  getSkillBuilder,
...
export function TaxCalculator({ skillBuilder, onComplete }) {
  ...
  useEffect(() => {
    console.log("TaxCalculator mounted", formData);
  }, [formData]);
  ...
  return (
    <div className="max-w-[1000px] mx-auto py-8">
      ...
      <div className="mb-8">
        {currentStep === 0 && (
          <ExpenseTracker formData={formData} setFormData={setFormData} />
        )}
        {currentStep === 1 && (
          <TaxSystemAnalysis
